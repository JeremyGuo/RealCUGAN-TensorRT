...
class FFMPEG_VideoReader:
    def __init__(
        ...
        self.depth = 4 if "a" in pix_fmt else 3
        self.c = 4 if "a" in pix_fmt else 3
        if "48" in pix_fmt:
    ...
    def read_frame(self):
        ...
        if len(s) != nbytes:
            ...
            if hasattr(np, "frombuffer"):
                result = np.frombuffer(s, dtype=self.dtype)
            else:
                result = np.fromstring(s, dtype=self.dtype)
            result.shape = (h, w, self.c)  # reshape((h, w, len(s)//(w*h)))
